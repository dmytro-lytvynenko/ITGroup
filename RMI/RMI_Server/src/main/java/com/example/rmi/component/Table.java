    ...
    public Table(Table table) {
        ...
        for (Column column : table.columns) {
            switch (ColumnType.valueOf(column.type)) {
                ...
                case TIME -> {
                    Column timeColumn = new TimeColumn(column.name);
                    this.columns.add(timeColumn);
                }
                case TIMEINVL -> {
                    Column timeInvlColumn = new TimeInvlColumn(column.name,((TimeInvlColumn) column).getMin(),((TimeInvlColumn) column).getMax());
